...
# Create ODE function
# The function must be in the quote and unquote. eg: "t^2-y^2"
def ode_function(t: float, y: float) -> float:
    ...
    def f(t: float, y: float) -> float:
        e = eval(sys.argv[1].replace("^", "**").replace(" ", ""))
        return e

    return f(t, y)


def tspan_init(args: str) -> tuple:
    """_summary_
...
# Solvers
def euler_method(f: float, tn: list, yn: list, h: float) -> float:
    return yn + h * f(tn, yn)


def rk4_method(f: float, tn: list, yn: list, h: float) -> float:
    k1 = f(tn, yn)
...
def solveODEs(f: float, tspan: list, y0: float, h: float, solver: float) -> float:
    """_summary_
    ...
    Args:
        ...
        solver (float): is any solver like Euler Method or RK4 Method
    """
    t = np.arange(tspan[0], tspan[1] + h, h)
